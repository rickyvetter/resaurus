function addReasonLanguageSupport(hljs) {
...
function applyCode(siteConfig) {
  if (!siteConfig.highlight) {
    siteConfig.highlight = {};
  }

  if (!siteConfig.highlight.theme) {
    siteConfig.highlight.theme = 'arduino-light';
  }

  if (!siteConfig.highlight.hljs) {
    siteConfig.highlight.hljs = addReasonLanguageSupport;
  } else {
    let otherHljs = siteConfig.highlight.hljs;
    // This should technically only provides defaults. This is an exception.
    siteConfig.highlight.hljs = function(siteConfig) {
      addReasonLanguageSupport(siteConfig);
      return otherHljs(siteConfig);
    }
  }
}

function applyColors(siteConfig) {
...
// applies sensible defaults for Reason sites generated with Docusaurus.
